...
impl PassageStore {
    fn new(store_path: Option<PathBuf>, identities_file: Option<PathBuf>) -> Result<Self, AppError> {
...
mod tests {
    ...
    fn passage_show_args_are_stable() {
        assert_eq!(passage_show_args("mail/account"), vec!["show", "mail/account"]);
    }
    ...
    fn lists_normalized_entries() {
        ...
        let mut entries = list_entries_with_extension(&store_dir, ".age").expect("entries should list");
        entries.sort();
